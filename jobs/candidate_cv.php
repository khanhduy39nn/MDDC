...
if(!empty($_POST)){	
	...
	$cv = "";
	if(!empty($_FILES['candidate_cv'])){                   			
		$name = time().$_FILES['candidate_cv']['name'];					
		if(move_uploaded_file($_FILES['candidate_cv']['tmp_name'], '../upload_files/candidate_cv/'.$name)){
			$cv = $name;		
		}			
	}
	
	$sql = "insert into jobs (id_user,title,description,type,location,dateadded,pay_from,pay_to,lfj,website_id,status,cv) 
		values (0,'".$title."','".$description."','".$type."','".$location."',".time().",".$pay_from.",".$pay_to.",".$lfj.",0,1,'".$cv."')";
	$rs = mysql_query($sql);
	if($rs){		
		$al = "Your CV upload successfully!";
	}else{		
		$al = "Fail! Please try again.";
	}
}
?>
<style type="text/css">
...
<i><?php if(!empty($al)){ echo $al; } ?></i>
<h3>Upload Candidate CV</h3>
<form method="post" action="" enctype="multipart/form-data" onsubmit="return checkValues()" >	
	...
	<tbody>	
		...
		<tr>
			...
			<td width="80%"><input type="text" id="title" name="title" value="" placeholder="Title Jobs" style="width:400px;color: #000 !important;"></td>
		</tr>
		<tr>
			<td width="20%" style="text-align:right !important; font-size:1.3em;">Pay from: </td>
			<td width="80%"><input name="pay_from" style="color: #000 !important;width:185px;" placeholder="Pay From" type="text" id="pay_from" value=""> To <input name="pay_to" value="" placeholder="Pay To" style="width:190px; color: #000 !important;"  type="text" id="pay_to"></td>
		</tr>		
		<tr>
			<td width="20%" style="text-align:right !important; font-size:1.3em;">Bussines Type: </td>
			<td width="80%">    	
			<select name="type" style="color: #000 !important;width:190px" value="">
...
<option value="Truck Transportation(Non Fuel)">---Truck Transportation(Non Fuel)</option>
			...
			or <input type="text" name="s_type"  placeholder="other bussines" style="width:185px;color: #000 !important;">
			</td>
		</tr>				
		<tr>
			<td width="20%" style="text-align:right !important; font-size:1.3em;">Location: </td>
			<td>
					<select name="location" style="color: #000 !important;width:195px;" >
...
or <input type="text" name="s_location"  placeholder="other locations" style="width:185px;color: #000 !important;">
</td>
		</tr>	
		<tr>
			<td width="20%" style="text-align:right !important; font-size:1.3em;">Content:</td>
			<td>
				<textarea id="description" name="description" cols="5" style="width:403px;color: #000 !important;" ></textarea>
			</td>
		</tr>	
		<tr>
			<td width="20%" style="text-align:right !important; font-size:1.3em;">Upload Candidate CV:</td>
			<td>
				<input id="candidate_cv" type="file" name="candidate_cv"  style="width:185px;color: #000 !important;">
			</td>
		</tr>	
		<tr>
			<td colspan="2">
				<center>
					<input type="submit"  value="Upload CV" style="background-color: #BEAA4B;width: 80px;height: 20px;color:#D8E378 !important;">
				</center>
			</td>
		</tr>
...
<script>
	function checkValues(){		
		...
		if(document.getElementById('message').innerHTML == ''){
			alert('Please upload your candidate CV');			
			return false;
		}
