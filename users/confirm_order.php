...
<tr style="border:0px solid #D8DB43 !important;">
	...
	if ($has_packages && $_REQUEST['pack']!='') { // has packages, and a package was selected...
		...
		if (can_user_get_package($_SESSION['MDS_ID'], $_REQUEST['pack'])) {
			...
			if($step['step'] >= 3){
				...
				$sql = "UPDATE temp_orders SET package_id='".$_REQUEST['pack']."', price='".$total."',  days_expire='".$pack['days_expire']."', currency='".get_default_currency()."', step = 3 WHERE session_id='".addslashes(session_id())."'";
			}
