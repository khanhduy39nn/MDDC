...
function place_temp_order($in_str, $price) {
	...
	$sql = "SELECT ad_id, block_info, step  FROM temp_orders WHERE session_id='".addslashes(session_id())."' ";
	...
	if($row['step'] >= 1){		
		...
		$sql = "REPLACE INTO `temp_orders` ( `session_id` , `blocks` , `order_date` , `price` , `quantity` ,  `days_expire`, `banner_id` , `currency` ,  `date_stamp` , `ad_id`, `block_info` ,`step`)  VALUES ('".addslashes(session_id())."', '".$in_str."', '".$now."', '0', '".$quantity."', '".DAYS_EXPIRE."', '".$_REQUEST['BID']."', '".get_default_currency()."',  '$now', '$ad_id', '$block_info' ,1);";
	}
