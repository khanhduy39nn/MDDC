    ...
    <div class="time-line">
      ...
      <div class="profile-topcard ">
        <div class="search-result-box">
          <?php   $resultc=  getRequestFriendList($user_id); ?>
          <h3>Respond to Your <?php echo mysql_num_rows($resultc); ?> Friend Requests</h3>
          <u class="search-result-list add_friend_request_list ">
          <?
          while ($row =  mysql_fetch_array($resultc)){
            echo '<li class="li-'.$row['notif_id'].'"><span><a href="'.working_domain.'?id='.$row['user_id_request'].'">'.$row['name'].'</a></span> <button class="accept-add-friend-request" onclick=acceptRequest("'.$row['user_id_request'].'",'.$row['notif_id'].') >Accept</button><button class="reject-add-friend-request" onclick=rejectRequest("'.$row['user_id_request'].'",'.$row['notif_id'].') >Reject</button></li>';
