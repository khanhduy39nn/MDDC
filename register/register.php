...
require "../config.php";
		if(!empty($_POST)){
			...
			if($flag == 0){
				$salary_bracket_from = 0;
				if(!empty($data['salary_bracket_from'])){
					$salary_bracket_from = $data['salary_bracket_from'];
				}
				$salary_bracket_to = 0;
				if(!empty($data['salary_bracket_to'])){
					$salary_bracket_to = $data['salary_bracket_to'];
				}
				$sql="INSERT INTO register (name, address, email,password,dateadded,phone,salary_bracket_from,salary_bracket_to,occupation,business_areas_of_interest) VALUES ('".$data['name']."', '".$data['address']."', '".$data['email']."','".md5($data['password_again'])."',".time().", '".$data['phone']."',".$salary_bracket_from.",".$salary_bracket_to.",'".$data['occupation']."','".$data['business_areas_of_interest']."')";
				$res = mysql_query($sql);
				...
				if($res){
					...
					$error = "<p style='color:red; font-style: italic;'>Fail. Please try again</p>";
				}
			}
			$_POST = NULL;
		}
