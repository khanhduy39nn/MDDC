...
/*Thien create new function send email with attacment 28082015*/
...
////////////////
/* Thien 19012016 */
function show_nav_status ($page_id, $step) {
	global $label;
	for ($i=1; $i <= 5; $i++) {
		if ($i==$page_id || $i <= $step) {
			$b1= "<b style='font-size:14px;'>"; $b2 = "</b>";
		} else {
			$b1= ""; $b2 = "";
		}
		if($label['advertiser_nav_href'.$i] != '' && $i <= $step){
			echo "<a style='text-decoration:none;' href='".$label['advertiser_nav_href'.$i]."'>";
		}
		echo $b1;
